...
@pytest.fixture
def mock_webpage_with_s(mocker):
...
@pytest.fixture
def mock_webpage_without_s(mocker):
...
@pytest.mark.asyncio
async def test_generate_webpages_with_s(mock_webpage_with_s, image_path):
...
@pytest.mark.asyncio
async def test_generate_webpages_without_s(mock_webpage_without_s, image_path):
    generator = GPTvGenerator()
    rsp = await generator.generate_webpages(image_path=image_path)
    logs.logger.info(rsp)
    assert "html" in rsp
...
@pytest.mark.asyncio
async def test_save_webpages_with_s(mock_webpage_with_s, image_path):
    generator = GPTvGenerator()
...
@pytest.mark.asyncio
async def test_save_webpages_without_s(mock_webpage_without_s, image_path):
    generator = GPTvGenerator()
    webpages = await generator.generate_webpages(image_path)
    webpages_dir = generator.save_webpages(image_path=image_path, webpages=webpages)
    logs.logger.info(webpages_dir)
